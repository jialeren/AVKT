...
def read_data(rootpath='/root/autodl-tmp/PKU-train', val_rate=0.1):
    ...
    keyword_dict = {'00001': 0, '00010': 1, '00100': 2, '01000': 3, '10000': 4, '00000': 5}
...
optimizer = torch.optim.SGD(KWS_A.parameters(), lr=1e-2)

for i in range(num_epoch):
    if i > 15:
        optimizer = torch.optim.SGD(KWS_A.parameters(), lr=1e-4)
    elif i > 10:
        optimizer = torch.optim.SGD(KWS_A.parameters(), lr=1e-3)
    ...
    for data in train_dataloader:
        ...
        writer.add_scalar('Loss7/train_loss_noise', ll.item(), total_train_step / 100)
    ...
    with torch.no_grad():
        ...
        writer.add_scalar('Loss7/test_loss_noise', lll.item(), i)
        acc = test_accuracy / len(test_dataset)
        print("测试集acc:{}".format(acc))
        bestacc = 0.75
        if acc > 0.75:
            if acc > bestacc:
                torch.save(KWS_A, '/root/PKU_A.pt')
                bestacc = acc

    writer.add_scalar('Acc7/train_acc_noise', acc0, i)
    writer.add_scalar('Acc7/test_acc_noise', acc, i)
